...
Bnd_Box GeomBndLib_Curve::Box(double theU1, double theU2, double theTol) const
{
  return std::visit(
    [theU1, theU2, theTol](const auto& theEval) -> Bnd_Box {
      using T = std::decay_t<decltype(theEval)>;
      if constexpr (std::is_same_v<T, std::monostate>)
      {
        return Bnd_Box{};
...
Bnd_Box GeomBndLib_Curve::BoxOptimal(double theU1, double theU2, double theTol) const
{
  return std::visit(
    [theU1, theU2, theTol](const auto& theEval) -> Bnd_Box {
      using T = std::decay_t<decltype(theEval)>;
