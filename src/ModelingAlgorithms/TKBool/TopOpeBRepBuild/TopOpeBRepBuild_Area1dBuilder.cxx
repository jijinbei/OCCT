...
#ifdef OCCT_DEBUG
void TopOpeBRepBuild_Area1dBuilder::DumpList(
...
#else
void TopOpeBRepBuild_Area1dBuilder::DumpList(
  const NCollection_List<occ::handle<TopOpeBRepBuild_Loop>>&)
{
#endif
}
  ...
  for (LS.InitLoop(); LS.MoreLoop(); LS.NextLoop())
  {
    ...
    if (!traitercommeblock)
    {
      ...
      for (AreaIter.Initialize(myArea); AreaIter.More(); AreaIter.Next())
      {
        ...
        if (aArea.IsEmpty())
          continue;
        state =
          CompareLoopWithListOfLoop(LC, L, aArea, TopOpeBRepBuild_LoopEnum::TopOpeBRepBuild_BLOCK);
    ...
    else
    {
      ...
      for (AreaIter.Initialize(myArea); AreaIter.More(); AreaIter.Next())
      {
        ...
        if (aArea.IsEmpty())
          continue;
        state = CompareLoopWithListOfLoop(LC,
                                          L,
                                          aArea,
                                          TopOpeBRepBuild_LoopEnum::TopOpeBRepBuild_ANYLOOP);
        if (state == TopAbs_UNKNOWN)
          Atomize(state, TopAbs_IN);
        Loopinside = (state == TopAbs_IN);
        if (Loopinside)
