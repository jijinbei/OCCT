...
const char* kSimdLevelName(BVH::SIMD::Level theLevel)
{
  switch (theLevel)
  {
    case BVH::SIMD::Level::Scalar:  return "Scalar";
    case BVH::SIMD::Level::SSE2:    return "SSE2";
    case BVH::SIMD::Level::AVX2:    return "AVX2";
    case BVH::SIMD::Level::AVX512:  return "AVX-512";
  }
  return "?";
}
...
{
  // ---------------------------------------------------------------------
  // 1. Build a deterministic random scene of 256 axis-aligned boxes inside
  //    the cube [-10, 10]^3.
  // ---------------------------------------------------------------------
  // Number of random AABBs in the scene. Override with OCCT_BVH_VIZ_PRIMS.
  int kNumPrims = 256;
  ...
  opencascade::handle<BVH_BoxSet<double, 3>> aBoxSet =
    new BVH_BoxSet<double, 3>(aBuilder);

  struct RawBox { float minX, minY, minZ, maxX, maxY, maxZ; };
  ...
  // Build() permutes the internal arrays via Swap(), so the index returned
  // by the BVH traversal is the *post-build* slot, not the original Add() id.
  // Re-collect boxes in their post-build order so aRawBoxes[idx] matches
  // whatever the acceptor reports.
  ...
  // ---------------------------------------------------------------------
  // 2. Collapse the binary tree into BVH4 (QuadTree) and BVH8 (OctTree).
  //    Convert both to <float, 3> so the SIMD traversal can consume them.
  // ---------------------------------------------------------------------
  const auto& aBinaryD = aBoxSet->BVH();
  opencascade::handle<BVH_Tree<double, 3, BVH_QuadTree>> aQuadD(
    aBinaryD->CollapseToQuadTree());
  opencascade::handle<BVH_Tree<double, 3, BVH_OctTree>> aOctD(
    aBinaryD->CollapseToOctTree());

  auto aQuadF = ConvertTreeToFloat<BVH_QuadTree>(aQuadD);
  auto aOctF  = ConvertTreeToFloat<BVH_OctTree>(aOctD);
  ...
  // ---------------------------------------------------------------------
  // 4. Run BVH4 and BVH8 traversal, collect the hit primitive indices.
  // ---------------------------------------------------------------------
  std::set<int> aHitsQuad, aHitsOct;
  ...
  BVH::SIMD::TraverseOct (aOctF,  aRay, aAccOct);

  // ---------------------------------------------------------------------
  // 5. Emit a self-contained HTML file with the data inlined.  The viewer
  //    pulls Three.js from a CDN and renders the scene in the browser.
  // ---------------------------------------------------------------------
  // The data block embedded in the HTML carries scene parameters only;
  // the boxes are regenerated client-side so a slider can change N live.
  std::ostringstream aData;
  aData << "{\n"
        << "  \"ray\": {\"origin\":[" << aOrigin.x() << "," << aOrigin.y()
        << "," << aOrigin.z() << "],\"dir\":[" << aDir.x() << "," << aDir.y()
        << "," << aDir.z() << "],\"length\":40},\n"
        << "  \"sceneBounds\": [-10, 10],\n"
        << "  \"sizeRange\":   [0.3, 1.2],\n"
        << "  \"initN\":       " << kNumPrims << ",\n"
        ...
        << "    \"benchBVH4\": \"Scalar 15.12 ns | SSE2 2.02 ns | AVX2 2.13 ns\",\n"
        << "    \"benchBVH8\": \"Scalar 35.16 ns | SSE2 4.06 ns | AVX2 3.10 ns\"\n"
        << "  }\n"
        << "}\n";
...
<head>
  <meta charset="utf-8">
  <title>OCCT BVH4 vs BVH8 traversal</title>
  <style>
    ...
    #info { position: absolute; top: 10px; left: 10px; padding: 12px 16px;
            ...
            border-radius: 6px; max-width: 320px; line-height: 1.4; }
    ...
    #bench { background: #000; padding: 6px 8px; margin: 4px 0;
             white-space: pre-wrap; font-size: 11px; }
  </style>
</head>
<body>
  <div id="info">
    <h3>BVH4 vs BVH8 traversal</h3>
    <div>SIMD level: <b><span id="simd">-</span></b></div>
    ...
    <label><input type="checkbox" id="showHits" checked>
      <span class="swatch" style="background:#f33"></span>Hit AABBs</label>
    <hr>
    <div>Throughput (million AABB-Ray tests / second):</div>
    <pre id="benchTable" style="font-size:11px;line-height:1.35;">
                BVH4        BVH8
  Scalar     265 M       228 M
  SSE2     1 980 M     1 970 M
  AVX2     1 878 M     2 580 M  &lt;- best
  ─────────────────────────────
  vs Scalar best: 11.3x  (BVH8 AVX2)
  vs SSE2 best:    1.31x (BVH8 wins)</pre>
    <div style="font-size:11px;color:#888;margin-top:6px;">
      Raw kernel timings: <span id="benchRaw">RayBox4 AVX2 = 2.13 ns / 4-AABB call &middot; RayBox8 AVX2 = 3.10 ns / 8-AABB call</span>
    </div>
    <div style="font-size:11px;color:#888;margin-top:8px;">
      ...
      result BVH4 and BVH8 traversal would return.)
    </div>
  </div>
  <canvas id="cv"></canvas>
  ...
  static const char* kHtmlSuffix = R"HTML(;
    ...
    const controls = new THREE.Group(); // placeholder
    ...
    // Helpers: thicker XYZ axes (cylinder + cone tip + sprite label)
    ...
    // ---- Mulberry32 PRNG (deterministic, matches no specific C++ seed but
    //      reproducible across slider moves so the same N gives the same
    //      scene every time) ---------------------------------------------
    ...
    // ---- Dynamic scene groups (rebuilt on slider change) -------------
    ...
    function rebuild(N) {
      ...
      for (let i = 0; i < N; ++i) {
        ...
        if (rayHitsBox(b)) {
          ...
          // Drop line from box centre to nearest point on ray.
    ...
    slider.addEventListener('input', () => {
      ...
      // Debounce expensive rebuilds while the user drags.
  ...
  {
    aOutPath = "/tmp/bvh8_visualization.html";
  }
  ...
  std::cout << "\n=== BVH4/BVH8 visualization ===\n"
            << "  primitives    : " << kNumPrims << "\n"
            << "  BVH4 visited  : " << aHitsQuad.size() << "\n"
            << "  BVH8 visited  : " << aHitsOct.size() << "\n"
            << "  SIMD level    : " << kSimdLevelName(BVH::SIMD::Detect()) << "\n"
            << "  HTML output   : " << aOutPath << "\n"
            << "  open with     : xdg-open " << aOutPath << "\n";

  // Numerical sanity check: for every hit AABB, print the box, its center,
  // and the perpendicular distance from the box center to the ray line.
  // (The actual hit test is the 3-axis slab test against the AABB itself,
  //  not against the center; this distance is purely a sanity check.)
  const float aDirLen = std::sqrt(aDir.x() * aDir.x() + aDir.y() * aDir.y()
                                  + aDir.z() * aDir.z());
  const BVH_Vec3f aDirN(aDir.x() / aDirLen, aDir.y() / aDirLen, aDir.z() / aDirLen);

  std::cout << "\n  ray origin = (" << aOrigin.x() << ", " << aOrigin.y()
            << ", " << aOrigin.z() << ")"
            << "  dir(normalised) = (" << aDirN.x() << ", " << aDirN.y()
            << ", " << aDirN.z() << ")\n\n";

  std::cout << "  hit AABBs (BVH8):\n";
  for (int idx : aHitsOct)
  {
    ...
    const float perpDist = std::sqrt((cx - fx) * (cx - fx) + (cy - fy) * (cy - fy)
                                     + (cz - fz) * (cz - fz));
    const float halfDiagonal = 0.5f * std::sqrt((b.maxX - b.minX) * (b.maxX - b.minX)
                                                + (b.maxY - b.minY) * (b.maxY - b.minY)
                                                + (b.maxZ - b.minZ) * (b.maxZ - b.minZ));
    std::cout << "    [" << idx << "] AABB = [" << b.minX << ", " << b.minY << ", "
              << b.minZ << "] ~ [" << b.maxX << ", " << b.maxY << ", " << b.maxZ << "]"
              << "  centre = (" << cx << ", " << cy << ", " << cz << ")"
              << "  t = " << t << "  perpDist = " << perpDist
              << "  halfDiag = " << halfDiagonal
